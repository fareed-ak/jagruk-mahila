...
import { REPRODUCTIVE_RIGHTS_DATA } from '../constants/reproductiveRightsData';
...
export default function Reproductive() {
    ...
    // Short header title per language
    const headerTitle = () =>
        ({ hi: 'प्रजनन अधिकार', ur: 'تولیدی حقوق' }[lang] || 'Reproductive Rights');
    ...
    const renderSubcategoryList = () => (
        ...
        >
            {REPRODUCTIVE_RIGHTS_DATA.subcategories.map((sub) => (
                <TouchableOpacity
                    key={sub.id}
            ...
            {current.level > 0 && (
                <View style={[styles.breadcrumb, isRTL && styles.breadcrumbRTL]}>
                    <Text style={styles.breadcrumbText}>
                        {t(REPRODUCTIVE_RIGHTS_DATA.label)}
                    </Text>
                    {current.level >= 1 && (
                        <>
                            <Ionicons
